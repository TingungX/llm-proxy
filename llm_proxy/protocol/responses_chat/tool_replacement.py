...
APPLY_PATCH_TOOL_DESCRIPTION: str = (
    "Apply edits using the apply_patch DSL. The input is a single string containing a patch.\n"
    "Overall structure (must match exactly):\n"
    "*** Begin Patch\n"
    "<file sections>\n"
    "*** End Patch\n\n"
    "File sections (one or more):\n"
    "*** Add File: <path>      (create new file; every line in body MUST start with '+')\n"
    "*** Update File: <path>   (edit existing file)\n"
    "*** Delete File: <path>   (remove file; no body)\n\n"
    "Update File details:\n"
    "- Optional control lines immediately after header:\n"
    "  *** Move to: <new_path>\n"
    "  *** at <line_number>\n"
    "- Then hunks begin with a single line: '@@'\n"
    "- Each subsequent line in the hunk MUST start with one of:\n"
    "  ' ' (context, must match file exactly)\n"
    "  '-' (minus, removal)\n"
    "  '+' (plus, addition)\n\n"
    "Strict matching rules:\n"
    "- All headers must start with exactly '***' (three asterisks), case-sensitive.\n"
    "- Begin/End Patch lines must match exactly.\n"
    "- Context lines in Update File must match the target file character-for-character "
    "(including whitespace and line endings).\n\n"
    "Common failure:\n"
    "\"Failed to find expected lines\" indicates the Update hunk context does not match the "
    "existing file exactly (even minor whitespace, ordering, or newline differences will break matching).\n"
)
